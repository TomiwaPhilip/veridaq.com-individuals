export function OutlineButtonSmall({ name }: { name: string }) {
  ...
  export function NoOutlineButtonSmall({ name, type }: { name: string, type: "submit" | "button" }) {
    return (
      <button type={type} className="veridaq-gradient text-white text-sm px-4 py-2 rounded-md border-transparent hover:bg-transparent hover:veridaq-gradient hover:border-gray-800">
        {name}
      </button>
    );
  }
