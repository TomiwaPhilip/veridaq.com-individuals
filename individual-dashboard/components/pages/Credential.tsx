...
import { getVeridaqURL } from '@/lib/actions/request.action';

export default function CredentialPage() {
    const searchParams = useSearchParams()
    const [verifyResult, setVerifyResult] = useState('Click the below button to verify your Veridaq');
    ...
    const handleVerify = async () => {
        try {
            ...
            const result = await getVeridaqURL(id)
            console.log(result)
            if (result) {
                setVerifyResult('Veridaq verified!');
